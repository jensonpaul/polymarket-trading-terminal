...
impl PredictionService {
    ...
    pub async fn run(
        ...
        info!(
            "PredictionService starting"
        );

        //
        // BTC feed owns its own reconnect loop.
        //
        let btc_feed =
            Arc::clone(&self.btc_feed);
        ...
        loop {
            ...
            {
                let slug =
                    slug_for_ts(window_ts);

                match get_or_fetch_token_ids(
                    &gamma,
                    &slug,
                )
                .await
